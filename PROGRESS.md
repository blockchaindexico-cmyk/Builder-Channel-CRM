...
| **Current phase** | Building — Milestone B (Lead Operations MVP): M03 done, M04 in progress |
| **Current module** | **M04 — Lead Management Core** (lead core done; import, export and intake API next) |
| **Overall progress** | 79 / 183 tasks (43%) |
...
| M04 | Lead Management Core | 🟨 In progress | 16 / 23 | 2026-09-25 | – | Q-01/02/03/05 defaults adopted (D-021); decisions D-022 → D-025. Open: import, export, intake API (M04-15, 16, 18–20, 22, 23) |
...
| B — Lead Operations MVP | M03–M05 | 🟨 (M03 ✅, M04 🟨) |
...
1. **Finish M04:** CSV/XLSX import with column mapping, preview, background job and error report (M04-18);
   CSV/XLSX export respecting data scope, incl. "export selected" in the bulk bar (M04-19, M04-15); intake API
   `POST /api/v1/leads` with API keys, idempotency and rate limits + API key settings and docs page (M04-20);
   duplicate checks on import/API (M04-16); `lead.import_completed` (M04-22); import/API tests (M04-23).
2. **M05 — Lead Assignment** next (needs Q-06, Q-08 or their proposed defaults).
3. Open a pull request so the GitHub Actions workflow runs for the first time (all its steps were executed locally).
4. Business confirmation still wanted for Q-01 → Q-05 (defaults implemented and configurable).
